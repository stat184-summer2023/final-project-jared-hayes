...
pitching<-pitching%>%
  mutate(IP=IPouts/3)%>%
  group_by(playerID)%>%
  summarise(wins=sum(W),total_IP=sum(IPouts),avg_ERA=mean(ERA),Games=sum(G))
```
...
batting<-batting%>%
  ...
  group_by(playerID)%>%
  summarise(hits=sum(H),HRS=sum(HR),average_average=mean(batting_average),Games=sum(G),At_bats=sum(AB))
```
...
combined_data<-combined_data%>%
  filter(game>30)
```
Combine new_data and HOF batting my name 


```{r}
master_batting<-master_batting%>%
  filter(Games>10)%>%
  filter(At_bats>100)
```
Try and filter out outliers due to small smaple size. 
```{r}
master_pit<-master_pitching%>%
  filter(Games>10)%>%
  filter(total_IP>100)
```
Again trying to filter out possible outliers due to small sample size. 
...
```
This graph shows that there is a clear correlation between innings pitches and wins. This is helpful because it shows me that wins matter more than innings pitched when predicting if a player makes it to the hall of time. 
...
ggplot(combined_data) +
  ...
  xlim(0,1)
```
This graph shows that it is hard to predict career performance off one year. This helps me answer my research question because it shows that career totals matter much more because it is hard to acheieve consistency. 
## Conclusion

Answer your research question using the data sets and visualizations you created.

I showed that wins and innings are equally important when determining if a plater makes it to the hall of fame. I also showed that innings pitched are not important because they are heavily correlated to wins. I showed that hits matters more than batting average to make it to the hall of time. I also showed that career stats do not predict stats in one year. Thus I answered my research question because I showed that career totals are better at predicting if a player makes it to the hall of fame when compared to rate stats.  